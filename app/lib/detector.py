...
class ObjectDetector:
    ...
    def __init__(self):
        # @param ["https://tfhub.dev/google/faster_rcnn/openimages_v4/inception_resnet_v2/1", "https://tfhub.dev/google/faster_rcnn/openimages_v4/inception_resnet_v2/1"]
    ...
    def run_detector(self, detector, path):
        ...
        for i in range(len(detection_scores)):
            ...
            name = result["detection_class_entities"][i].decode("ascii")
            if score > 0.5:
                response.append({"name": name.lower(), "score": str(score)})
        return response

    def detect_img(self, image_url):        
        image_path = self.download_and_resize_image(image_url, 640, 480)
        response = self.run_detector(self.detector, image_path)        
        return response